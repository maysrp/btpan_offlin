	...
	*/
	class UserModel extends Model
	{
		...
		function oid($file){
			...
			$o_json=json_decode($user['num_all'],true);
			if(in_array($hash, $o_json)){
				$re['status']=true;
			}else{
				$o_json[]=$hash;
				$save['uid']=$_SESSION['uid'];
				$save['num_all']=json_encode($o_json);
				$save['now_num']=$user['now_num']+1;
				$save['now_size']=$user['now_size']+$file['size'];
				$this->save($save);
				$re['status']=false;
